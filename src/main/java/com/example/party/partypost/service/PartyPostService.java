...
public class PartyPostService implements IPartyPostService {
    ...
    public DataApiResponse<PartyPostListResponse> findPartyList(User user, int page) {
        List<Blocks> blockedList = blockRepository.findAllByBlockerId(user.getId());
        ...
        List<PartyPostListResponse> returnByPartyPost = filteringPosts(blockedList, pageResponse).stream()
                .map(PartyPostListResponse::new).collect(Collectors.toList());
        return DataApiResponse.ok("모집글 전체 조회 완료", returnByPartyPost);
    }
